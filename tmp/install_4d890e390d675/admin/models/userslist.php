...
class McmModelUserslist extends JModel{
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__users` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,name,last_name,username,email,password,usertype,block,sendEmail,gid,registerDate,lastvisitDate,activation,params'); // array('id', 'ordering', 'published'); 

		// retrive ordering info
		$filter_order = $app->getUserStateFromRequest('com_mcmfilter_order', 'filter_order', $default_order_field);
		$filter_order_Dir = strtoupper($app->getUserStateFromRequest('com_mcmfilter_order_Dir', 'filter_order_Dir', 'ASC'));
	...
	private function _buildQueryWhere() {
		...
		$search = $app->getUserStateFromRequest('com_mcmsearch', 'search', '');
		
	    if (!$search) return '';
		
		$allowedSearch = explode(',', 'name,last_name,username,email,password,usertype,activation'); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__users` t1 ' . $this->_buildQueryWhere() );
