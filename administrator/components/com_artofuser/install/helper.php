<?php
/**
 * @version		$Id: helper.php 537 2011-01-15 02:22:57Z eddieajau $
 * @package		NewLifeInIT
	...
	public static function installModules(&$installer)
	{
		...
		if (is_a($modules, 'JSimpleXMLElement') && count($modules->children())) {
			foreach ($modules->children() as $module)
			{
				...
				$mAccess	= $module->attributes('access');
				...
				if ($mtitle && $mposition) {
					...
					if (!$installed) {
						...
						$row->iscore	= 0;
						$row->access	= ($mclient->id) == 1 ? 2 : (int) $mAccess;
	...
	public static function installPlugins(&$installer)
	{
		...
		if (is_a($plugins, 'JSimpleXMLElement') && count($plugins->children())) {
			...
			foreach ($plugins->children() as $plugin)
			{
				...
				$pState		= $plugin->attributes('published');
