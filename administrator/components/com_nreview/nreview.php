...
$controllers = explode(',', 'reviewslist');
if(!JRequest::getWord('controller')){
	JRequest::setVar( 'controller', $controllers[0] );
}
foreach($controllers as $controller){
	$link = JRoute::_("index.php?option=com_nreview&controller={$controller}");
	$selected = ($controller == JRequest::getWord('controller'));
	JSubMenuHelper::addEntry(JText::_( 'MENU' . $controller ), "index.php?option=com_nreview&controller={$controller}", ($controller == JRequest::getWord('controller')));
}
