 ...
 * @subpackage 	Nreview
 */
class NreviewModelReviewslist extends JModel{
	/**
	 * Reviewslist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT t1.* FROM `#__reviews` t1 ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,name,address,reservations,quicktake,review,notes,smoking,credit_cards,cuisine,avg_dinner_price,review_date,published'); // array('id', 'ordering', 'published'); 

		// retrive ordering info
		$filter_order = $app->getUserStateFromRequest('com_nreviewfilter_order', 'filter_order', $default_order_field);
		$filter_order_Dir = strtoupper($app->getUserStateFromRequest('com_nreviewfilter_order_Dir', 'filter_order_Dir', 'ASC'));
	...
	private function _buildQueryWhere() {
		...
		$search = $app->getUserStateFromRequest('com_nreviewsearch', 'search', '');
		
	    if (!$search) return '';
		
		$allowedSearch = explode(',', 'name,address,reservations,credit_cards,cuisine'); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__reviews` t1 ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__reviews` ORDER BY `$fieldName`');
		$options = array();
